...
def inference(input_tensor, avg_class, weights1, biases1, weights2, biases2):
    ...
    if avg_class == None:
        ...
        ###用avg_class.average函数来计算得出变量的滑动平均值
        layer1 = tf.nn.relu(
...
def train(mnist):
    ...
    weights1 = tf.Variable(
                tf.truncated_normal([INPUT_NODE, LAYER1_NODE], stddev=0.01)
    )
    biases1 = tf.Variable(
                tf.constant(0.5, shape=[LAYER1_NODE])
    )
    weights2 = tf.Variable(
                tf.truncated_normal([LAYER1_NODE, OUTPUT_NODE], stddev=0.01)
    )
    biases2 = tf.Variable(tf.constant(0.5, shape=[OUTPUT_NODE]))
    ##########################################不用滑动平均模型的y
    y = inference(x, None, weights1, biases1, weights2, biases2)
    
    ###定义存储训练轮数的变量。这个变量不需要计算滑动平均值，所以指定这个变量为不可训练变量trainable=False
    ### 在使用tensorflow训练神经网络时，一般会将代表训练轮数的变量指定为不可训练的参数
    global_step = tf.Variable(0, trainable=False)
    variable_average = tf.train.ExponentialMovingAverage(
                    MOVIN_AVERAGE_DECAY, global_step
    )
    
    ### tf.trainable_variable()  是返回图上集合GraphKeys.TRAINABLE_VARIABLES中的元素（除了trainable=False）
    #### 进行滑动平均
    variables_avrages_op = variable_average.apply(
                    tf.trainable_variables()
    )
    ##########################################用了滑动平均模型的y
    ### 调用影子变量 shadow_variables 需要利用variable_average类的average方法
    average_y = inference(x, variable_average, weights1, biases1, weights2, biases2)
    ###计算损失函数
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=average_y, labels=tf.argmax(y_, 1))
    ...
    with tf.control_dependencies([train_step, variables_avrages_op]):
        train_op = tf.no_op(name='train')
    ###等价于 train_op = tf.group(train_step, variables_avrages_op)
    
    ### 计算准确率
    
    #### tf.argmax() 的第二个参数“1”表示选取参数只在第一个维度进行。
...
#                 _, loss_value, step = sess.run(train_op, feed_dict={x:xs, y_:ys})
#             print('After %d training steps, train accuaracy is %g' %(epoch, loss_value))
        ...
        ###每1000次打印结果
            ...
            if i%1000 == 0:
                validate_acc = sess.run(accuaracy, feed_dict=validate_feed)
                print('After %d training steps, validation accuaracy is %g' %(i, validate_acc))
            
        test_acc = sess.run(accuaracy, feed_dict=test_feed)
        print('After %d training steps, test accuaray is %g' %(TRAINING_STEPS, test_acc))
## 主程序入口
def main(argv=None):
    mnist = input_data.read_data_sets("/tmp/data", one_hot=True)
    train(mnist)

#tensorflow提供一个主入口程序,tf.app.run会调用上面的main函数
